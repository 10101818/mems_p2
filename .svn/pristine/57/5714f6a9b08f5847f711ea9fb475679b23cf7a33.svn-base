				...
				<div class="left box box3">
                    ...
                    <div class="list-box-container">
                        <div :class="(chart3.data.length>0 && chart3.data[0].name=='暂无报修'?'list-box3 flex-center':'list-box3')">
							<div class="list3_title flex-center">{{list3_title}}</div>
                ...
                <div class="right box box4">
					...
					<div class="setting">
						...
						popper-class="datepicker-poper">
							...
							<el-button type="text" slot="reference" style="color:#ccc">
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeDate(){
				//debugger
				this.date= this.date || ((localStorage.getItem("date"))?new Date(localStorage.getItem("date")) :new Date()) 
					...
					for (let i = 0; i < data.length; i++) {
						...
						if (ret == -1) {
                            _list.push({
                                ...
                                list: [
                                    {
                                        name: `${data[i].EquipmentName}[${data[i].EquipmentOID}]请求${data[i].RequestType.Name}`,
			...
			renderChart1() {
				...
				if(!Object.keys(_this.chart1_origin).length )
				...
				this.chart1.data.filter(info=> {
					if(filter != "" && filter != "all")
						return info.type == filter
					else return true
				}).sort(sortFunc).forEach(function(data, dataIndex) {
			...
			renderChart3() {
				...
				if(!Object.keys(_this.chart3_origin).length )
