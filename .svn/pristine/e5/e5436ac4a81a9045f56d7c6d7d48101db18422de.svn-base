...
namespace DataImport
{
    public partial class MainForm : Form
    {
        ...
        private const string SHEET_NAME_FUJICLASS1 = "FujiClass1";
        private const string SHEET_NAME_FUJICLASS1_EQPTYPE = "FujiClass1EqpType";
        private const string SHEET_NAME_FUJICLASS2 = "FujiClass2";
        private const string SHEET_NAME_FUJICLASS2_EQPTYPE = "FujiClass2EqpType";
        private const string SHEET_NAME_COMPONENT = "Component";
        private const string SHEET_NAME_CONSUMABLE = "Consumable";
        ...
        private List<string> _SheetNames = new List<string> { SHEET_NAME_SUPPLIER, SHEET_NAME_EQUIPMENT, SHEET_NAME_CONTRACT, SHEET_NAME_CONTRACT_EQUIPMENT,
                                                              ...
                                                              SHEET_NAME_DISPATCH_REPORT, SHEET_NAME_REPORT_ACCESSORY, SHEET_NAME_INCOME, SHEET_NAME_FUJICLASS1, 
                                                              SHEET_NAME_FUJICLASS1_EQPTYPE, SHEET_NAME_FUJICLASS2, SHEET_NAME_FUJICLASS2_EQPTYPE, SHEET_NAME_COMPONENT, SHEET_NAME_CONSUMABLE};
        ...
        private bool ImportData(Dictionary<string, List<EntityInfo>> dicList)
        {
            try
            {
                ...
                foreach (string sheetName in _SheetNames)
                {
                    ...
                    else if (SHEET_NAME_FUJICLASS1.Equals(sheetName))
                        importDao.ImportFujiClass1(dicList[sheetName]);
                    else if (SHEET_NAME_FUJICLASS1_EQPTYPE.Equals(sheetName))
                        importDao.ImportFujiClass1EqpType(dicList[sheetName]);
                    else if (SHEET_NAME_FUJICLASS2.Equals(sheetName))
                        importDao.ImportFujiClass2(dicList[sheetName]);
                    else if (SHEET_NAME_FUJICLASS2_EQPTYPE.Equals(sheetName))
                        importDao.ImportFujiClass2EqpType(dicList[sheetName]);
                    else if (SHEET_NAME_COMPONENT.Equals(sheetName))
                        importDao.ImportComponent(dicList[sheetName]);
                    else if (SHEET_NAME_CONSUMABLE.Equals(sheetName))
                        importDao.ImportConsumable(dicList[sheetName]);
                }
                ...
                foreach (string sheetName in _SheetNames)
                {
                    ...
                    foreach (DataRow dr in dicTable[sheetName].Rows)
                    {
                        if (SHEET_NAME_SUPPLIER.Equals(sheetName))
                            ...
                            entityInfos.Add(new FujiClass1Info(dr));
                        else if (SHEET_NAME_FUJICLASS1_EQPTYPE.Equals(sheetName))
                            entityInfos.Add(new FujiClass1EqpType(dr));
                        else if (SHEET_NAME_FUJICLASS2.Equals(sheetName))
                            entityInfos.Add(new FujiClass2Info(dr));
                        else if (SHEET_NAME_FUJICLASS2_EQPTYPE.Equals(sheetName))
                            entityInfos.Add(new FujiClass2EqpType(dr));
                        else if (SHEET_NAME_COMPONENT.Equals(sheetName))
                            entityInfos.Add(new ComponentInfo(dr));
                        else if (SHEET_NAME_CONSUMABLE.Equals(sheetName))
                            entityInfos.Add(new ConsumableInfo(dr));
                    }
        ...
        private void AutoGenerateRequests()
        {
            ...
            DateTime minDate = equipmentInfos.Min(t => t.InstalDate);
        ...
        private void GenerateRequestRepair(List<EquipmentInfo> equipmentInfos, ref int nextRequestId, ref int nextDispatchId, DateTime fromDate)
        {
            ...
            List<EquipmentInfo> toProcessEquips = (from EquipmentInfo temp in equipmentInfos
                                                   where temp.InstalDate <= fromDate && (temp.ScrapDate == DateTime.MinValue || temp.ScrapDate >= monthEnd)
                                                   select temp).ToList();
            ...
            while(tryCount < totalEquips * 2)
            {
                ...
                if (!equipIndexs.Contains(rdIndex))
                {
                    ...
                    requestInfo.Priority.ID = 1;
                    requestInfo.MachineStatus.ID = 1;
                    requestInfo.RequestDate = GetRandom(fromDate, toProcessEquips[rdIndex].InstalDate, toProcessEquips[rdIndex].ScrapDate, monthEnd);
                    requestInfo.RequestDate = requestInfo.RequestDate.AddHours(this.rd.Next(8, 14)).AddMinutes(this.rd.Next(0, 60));
        ...
        {
            List<EquipmentInfo> toProcessEquips = (from EquipmentInfo temp in equipmentInfos 
                                                   where temp.InstalDate <= fromDate && (temp.ScrapDate == DateTime.MinValue || temp.ScrapDate.Year >= fromDate.Year) select temp).ToList();
            ...
            foreach(EquipmentInfo info in toProcessEquips)
            {
                ...
                requestInfo.DealType.ID = 1;
                requestInfo.Priority.ID = 1;
                requestInfo.RequestDate = GetRandom(fromDate, info.InstalDate, info.ScrapDate, yearEnd);
