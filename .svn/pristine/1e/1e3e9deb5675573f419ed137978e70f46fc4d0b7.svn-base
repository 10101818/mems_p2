		...
		<div class="content">
			<div class="top">
				<div class="left box box1">
					<div class="top-list-box">
                        ...
                        <div class="item">
                            <div class=" center-center value">{{(overview.EquipmentAmount==null)?"Loading":((overview.DepreciationRate*100).toFixed(1)+"%")}}</div>
						...
						<div class="list-box1">
							<div class="list">
                                ...
                                <div :class="(totalIncome-totalIncomeLast)>0?'status up green':'status down red'">
                                    <div class=" center-center value">
                                        <span v-if="totalIncomeLast == 0">0.0%</span>
                                        <span v-else>{{(totalIncome==null)?"Loading":Math.abs(Number((totalIncome-totalIncomeLast)*1.0/totalIncomeLast*100).toFixed(1))}}%</span>
                            ...
                            <div class="list">
                                ...
                                <div :class="(totalExpense-totalExpenseLast)>0?'status up green':'status down red'">
                                    <div class=" center-center value">
                                        <span v-if="totalExpenseLast == 0">0.0%</span>
                                        <span v-else>{{(totalExpense==null)?"Loading":Math.abs(Number((totalExpense-totalExpenseLast)*1.0/totalExpenseLast*100).toFixed(1))}}%</span>
                                        <span class="iconfont icon-shang"></span>
                                    </div>
                                    <div class=" center-center text">
                                        同比去年
			...
			<div class="bottom">
                ...
                <div class="right box box4">
					<KpiList :masterDate="date" ref="kpi" class="kpi"></KpiList>
				</div>
			</div>
		</div>
	</div>
...
<script>
    ...
    import {getRem} from '@/common/scripts/common'
			...
			renderChart1() {
				...
				dv.source(result)
					.transform({
						...
						order: sort=='seq'?'ASC':'DESC', // 默认为 ASC，DESC 则为逆序
					})
				...
				_this.chart1_origin = new G2.Chart({
					container: 'chart1',
					forceFit: result.length <= 20 ? true : false,
					height: document.querySelector('#chart1').clientHeight,
					width: result.length * 48,
