        ...
        public int SaveContract(ContractInfo contract, List<UploadFileInfo> files, UserInfo userInfo)
        {
            if (contract.ID > 0)
            {
                ...
                if (dtField.Rows.Count > 0)
                {
                    ...
                    this.auditManager.AddAuditLog(userInfo.ID, BusinessObjects.Domain.AuditHdrInfo.AuditObjectTypes.Contract, contract.ID, dtField);
                }
            }
            else
            {
                ...
                if (files != null && files.Count > 0)
                {
                    foreach (UploadFileInfo file in files)
                    {
                        ...
                        file.ObjectName = ObjectTypes.Contract;
