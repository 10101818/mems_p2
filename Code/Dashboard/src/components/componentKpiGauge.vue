  ...
  export default {
    ...
    methods: {
      ...
      link(item) {
        ...
        if (item.name === '校准率')
          this.params.requestType=5
        if (item.name === '保养率')
          this.params.requestType=2
        if (item.name === '巡检率')
          this.params.requestType=4
        let paramStr="?a=0"
        for(let i in this.params){
          paramStr += "&"+ i +"=" +this.params[i]
        }
        window.location.href = BASE_URL+'/Request/RequestList' + paramStr;//KPI跳转位置
      },
      getList() {
        console.log(this.masterDate)
        let params = {date :this.masterDate}
        return new Promise(resolve => {
          kpiList(params).then(res => {//params
            let data = res.Data;
            this.chart.value = (Math.floor(data.BootRate.Present * 100)/10).toFixed(1)
            this.chart.target = (data.BootRate.Default * 10).toFixed(1) * 1
            this.list.forEach(n => {
              if (data[n.key].Plans == 0) {
                n.value = 0
              } else {
                n.value = ( 100 * data[n.key].Done / data[n.key].Plans).toFixed(1) * 1
              }
      ...
      renderChart() {
        ...
        Object.keys(_this.chart_origin).length && !_this.chart_origin.destroyed && _this.chart_origin.destroy()
          _this.chart_origin = new G2.Chart({
            container: 'kpi-chart',
            forceFit: true,
        ...
        function draw(data) {
		  ...
		  if (document.querySelector("#kpi-chart") != null){
			  let top = document.querySelector("#kpi-chart").parentElement.parentElement.parentElement
			  let varticalOffSet = [...top.classList].some(temp => temp == "top")?"72%":"75%"
			  // 绘制指标数字
			  chart.guide().html({
				position: ['50%', varticalOffSet],
