...
namespace MedicalEquipmentHostingSystem.Controllers
{
    ...
    public class EquipmentController : BaseController
    {
        ...
        public JsonResult QueryEquipments(int status, int warrantyStatus, int departmentID, string filterTextName, string filterTextSerialCode, bool useStatus, string filterField, string filterText, string sortField, bool sortDirection, int currentPage = 0, int pageSize = ConstDefinition.PAGE_SIZE)
        {
            ...
            try
            {
                ...
                if (currentPage > 0)
                {
                    int totalRecord = this.equipmentDao.QueryEquipmentsCount(status, warrantyStatus, departmentID,  filterTextName,  filterTextSerialCode,useStatus, filterField, filterText);
                    result.SetTotalPages(totalRecord, pageSize);

                    if (totalRecord > 0)
                        infos = this.equipmentDao.QueryEquipments(status, warrantyStatus, departmentID, filterTextName, filterTextSerialCode,useStatus, filterField, filterText, sortField, sortDirection, result.GetCurRowNum(currentPage, pageSize), pageSize);
                }
                else
                {
                    infos = this.equipmentDao.QueryEquipments(status, warrantyStatus, departmentID, filterTextName, filterTextSerialCode,useStatus, filterField, filterText, sortField, sortDirection);
                }
